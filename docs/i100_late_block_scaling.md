...
version of the twin, it is a different, damaging init. **`ftbanaf` (flat top) final 76.41** (train loss 2.207), 0.2 below `ftbana`: the top/middle write contrast
contributes nothing detectable on this baseline.
...
block 0 (`plots/cache/verify/lr_match_multipliers.json`). It tested whether step sizes *alone* carry the effect; they
...
**Reference prefix moved to blocks 0-7 (2026-09-17; set up and verified, not launched).** The full reconstruction (scales +
LN statistics + block-0 common write + sink + gate; statement and code walk in `docs/proc_init_recipe.md`) is now prepared
against the 0-7 prefix arms `ftb4i` (kdyck 79.89, n=3; ksd 80.05, n=1), the split several late-lever arms use and the only
ksd prefix that exists: `ftbanaperg0b7` (kdyck) and `ftbanakperg0b7` (ksd), second-moment-only bases `ftbanapeb7` /
`ftbanakpeb7`; specs `vitbase_runs/profile_<arm>.json` from `extract_profile.py --blocks 0-7 --gain_fold exact --exact
[--qk_sink --fc1_gate --common_write]`, run scripts `run_train_<arm>.sh` (`--init_method_scaled_blocks 0..7`). Computed, not
assumed: the 0-7 specs equal the 0-8 specs minus block 8 (148 numbers, no shared value differs); through `main.py` (job
29727574) all 96 tensors of blocks 0-7 are bit-identical to the 0-8 arms' and everything outside blocks 0-7 equals `ftb4i`'s
init dump (56/56); `verify_recipe_statement.py` and `verify_joint_statistics.py` (spec and dump mode) PASS for both tasks;
2-rank fp16 smoke: `[joint statistics] 15 calibrated tensors identical on all 2 ranks: True`, loss 7.08 -> 6.94 (kdyck),
7.03 -> 6.94 (ksd). Both verifiers and `plot_reconstruction.py` now take the block range from the spec. New measured
difference: the stream entering block 8 has rms 53.0 under the kdyck prefix and 5.9 under the reconstruction (second
moments 2.9, random 1.35; ksd 30.9 vs 4.3), so the random tail's writes are 5-6% of the stream instead of 0.5-0.7%;
`ftbanap` (stream 2.8, 80.24) says the large stream is not needed on kdyck. The 0-8 arms (`ftbanaperg0`, `ftbanakperg0`)
stay set up; the 0-8 evidence (`ftbanap`, `ftbanal`, `ftbanac`, `ftbanape`, `ftbanapx`, ksd recipe arms) is not transferred.

**`ftbc7p` (kdyck profile-only cell, epoch 113) RELEASED (2026-09-23 13:25, Simon: run it further, on H200).** Its shared-H200 primary 29745252 is back in the queue at the top of our shared jobs (continuation 29745253 chained; checkpoint 113 verified resumable), plus a group-H200 hand-over (nodes of our own shared runs excluded). The other three single-component cells (`ftbck7s` 189, `ftbck7g` 147, `ftbck7p` 78) stay held for now.

**FINAL `ftb4kc1` (full ksd checkpoint + block-7 align loss) 79.30 (2026-09-23 12:44, last epoch; train loss 2.332).** -0.47 against `ftb4k` 79.77, -0.59 against C ksd; the predicted crossing (+4.3 at epoch 49, level at 199, -0.5 from 249 on). Its no-op continuation 29757189 was cancelled (it would also have outranked `ftbc7l`'s continuation for the 14:05 group slot). The freed group slot went to `r0frzl`'s hand-over 29760132 at 12:44 (dlc2gpu18; resumed from checkpoint 190, the shared job 29760130 cancelled at epoch ~195 as designed).

**`r0frzl` mid-run lens + prediction (2026-09-23 11:10, epoch 164).** Block-7 lens 61 at epoch 29 and 65 at 159 (random 49 -> 35, C 7 -> 0.5, freeze arm 0.3): the reverse freeze forms the class in blocks 0-7 first, the opposite order to every winner; accuracy tracks C's curve (+0.24 over `r0` at 164 with C's fit deficit) but without the late blocks' late class formation that carries C's surge. Prediction written into plan section 5e: random-like, 78.0 +- 0.5. `r0frzl` was added to `plots/verify/wandb_layerwise.py` (ARMS) and its per-block accuracies pulled into the cache.

**FINALS `ftbc7ps` (P + S kdyck) 79.79 and `ftbck7a1g` (A1 ksd + LayerNorm gains) 79.36 (2026-09-23 09:48 / 09:43, last epoch).** P+S sits with the other kdyck pairs (P+G 79.92, S+G 79.57), 0.58 below C; the LayerNorm gains cost 0.23 against `ftbck7a1` (plan section 5e). Their no-op continuations were cancelled (`ftbck7a1g`'s 29765167; `ftbc7ps` finished inside its continuation 29765166). Queue events: `ftb3bb`'s shared-H200 hand-over wrapper 29765767 started at 09:29 (dlc2gpu21), took the run over from the L40S fallback (cancelled with its continuation, as designed) and resumed from the epoch-1 checkpoint: all three measured-profile arms are on H200 now. `ftbc7c1` (kdyck C1, epoch 234) was PREEMPTED again at 10:04 by a group-tier job; the requeue reset its priority below the released `ftbc7p`, so `ftbc7p` (pending since its own preemption at 07:50) was held again at 10:06 -- `ftbc7c1` takes the next shared slot.

**Measured-profile cell: starts and two launch-pattern fixes (2026-09-23 07:50-08:36).** `ftb3bpl` (29765171) and `ftb3bsl` (29765177) started on the shared H200 partition at 07:50 (dlc2gpu24, 4 GPUs each; `ftbc7p` was requeued at the same time and sits at the top of the shared queue at epoch 104). `ftb3bpl`'s L40S fallback wrapper ran at 07:55 and exited correctly (run already on a higher tier), but that made its `afterany` continuation eligible: a plain resume job that would have trained into the running run's directory -> cancelled (29765176). `ftb3bb`'s L40S fallback (29765226) started at 07:58, from scratch under results id 29765222; that left its plain shared-H200 primary (29765222) pending with the same hazard -> cancelled with its continuation and replaced by a shared-H200 hand-over wrapper (29765767, continuation 29765768), which will take the run over properly when a shared slot comes. The fallback had also taken the group L40S slot that the continuations of `ftbc7ps` (epoch 294, five epochs left) and `ftbc7g` (257) were waiting for: nice 200 does not help when the continuations become eligible seconds before the slot frees (both primaries hit their 23.5-h limits at 07:54/07:55). `ftb3bb` was requeued at 08:35 (epoch 1, resumes from its checkpoint later) so those continuations start first; the group L40S cap is shared with a colleague's 19 GPUs, so we have three slots there, not four. Lesson for the launch pattern: a plain run script as the primary is only safe when nothing else can start earlier; when a fallback wrapper starts first, the primary must become a wrapper, and continuations of a wrapper that exited without running must be cancelled.

**MEASURED-PROFILE CELL LAUNCHED, all three arms (2026-09-23 06:38-06:42, Simon: "launch all of them if they're verified, on H200 ideally").** `ftb3bpl` results id 29765171 (shared-H200 primary, continuation 29765172; group-H200 hand-over 29765173 + 29765174; group-L40S fallback 29765175 + 29765176 at nice 200 so no running arm's continuation is starved), `ftb3bsl` 29765177 (cont 29765178; group hand-over 29765179 + 29765180; no L40S fallback for the optional arm), `ftb3bb` 29765222 (cont 29765223; group hand-over 29765224 + 29765225; L40S fallback 29765226 + 29765227). `ftb3bb`'s init was checked through main.py with the run script's own flags first (job 29765170, one L40S GPU, SSD data copy, bf16): bit-identical to `ftb3b_s0.pth` in all nine scaled tensors and timm seed 0 in the other 143 (the SSD copy draws the same calibration images as the HDD path). Expected starts: `ftb3bpl` on the group L40S fallback ~09:15 (when `ftbc7ps` ends), first group-H200 slot ~14:40 (`ftb4c1` ends; `r0frzl`'s group hand-over is ahead for the ~15:10 slot), `ftb3bb` on L40S ~10:35, `ftb3bsl` on H200 when a further slot frees (~21:00). Read-out per the brief: last epoch from log.txt, third 2x2 table in `docs/proc_init_recipe.md` section 13 and `docs/code_review_plan.md` section 1; caveats bf16-vs-fp16 (until `ftb3bb`), compounding (a warm-up divergence of `ftb3bpl` is a result), nominal step match.

**FINAL `ftbck7c1` (C ksd + align loss on the block-7 lens, epochs 0-49) 79.19 (2026-09-23 ~01:25, last epoch; train loss 2.280).** C ksd 79.90 (2.374): -0.71, at the edge of the pairwise resolution, with the predicted crossing (+1.4 at epoch 49, 0.0 at 199, -0.7 at 299) and a smaller fit deficit throughout (plan section 5e). Its group H200 slot went to `ftb4c1`'s queued group hand-over 29757190 (dlc2gpu21, from its epoch ~185 checkpoint; the shared primary 29754129 was cancelled by the wrapper). Overnight queue events: `r0frzl` started 22:38 (shared, dlc2gpu22; watcher released the holds), `ftbc7c1` (29753656, epoch 189) was requeued at ~02:00 and the freed shared slot went to the released `ftbc7p` (29745252, running since ~04:07, epoch 83) -- the requeue trap again; at 06:35 `r0sup` (29751230) and `ftbck7p` (29745250) were HELD again so the next shared slot goes to `ftbc7c1` (`ftbc7p` left running, per the standing rule). Mistake, corrected within minutes: at 06:33 the L40S continuations of `ftbc7ps` (29754343) and `ftbck7a1g` (29754349) were cancelled on a misread of two finished ksd arms' results ids (29745244 = `ftbck7ps`, 29745246 = `ftbck7pg`, both long final) as theirs; both primaries were still running (epochs 285 and 279, 1.4 h and 3.6 h left on their limits), and the continuations were re-created at 06:36 as 29765166 / 29765167 (same settings: group L40S, 8 GPUs, nice 40, afterany on the primaries, MAX_RETRIES 12). No run lost anything.

**MEASURED-PROFILE STEP-SIZE CELL `ftb3bpl` SET UP AND VERIFIED (2026-09-22 17:30-18:10; brief `docs/late_lever_measured_profile_brief.md`; NOT launched, waiting for the go).** The late-lever 2x2 at proc's measured late-block profile instead of the flat 1.4 (`docs/proc_init_recipe.md` section 13, last paragraph). `ftb3b`'s launched init reproduced as a dump with its own flags (`results/init_dumps/ftb3b_dump.cmd`, job 29760435, one L40S GPU, 7 min; targets printed 1.382 / 2.092 / 0.772 attention, 4.817 / 4.480 / 0.522 MLP against the launched 1.380 / 2.098 / 0.777, 4.757 / 4.459 / 0.526); multipliers fitted (`plots/verify/fit_late_multipliers.py`, residual 2.5e-8): v = proj 2.988 / 9.515 / 17.747, fc2 32.14 / 331.07 / 223.23 for blocks 9-11 (fc2 3.4x / 12x / 2.7x `ftbrho`'s; block 11 larger, not smaller, because its ratio is taken against the stream blocks 9-10 have already inflated). Files: `profile_ftb3bpl.json`, `lrscale_ftb3bpl.json`, `lrscale_ftb3bsl.json`; scripts `run_train_ftb3bpl.sh` (from `ftbrhoplv`, bf16), `run_train_ftb3bb.sh` (the `ftb3b` init on the current template, bf16, SSD data copy, one seed), `run_train_ftb3bsl.sh` (from `ftbrhoslv`), all with `--analysis_ckpt_dense_until 60 --analysis_ckpt_every 10`. Verifier `plots/verify/verify_late_trio_3b.py` (job 29760562): A-E PASS (reconstruction = dump to 1.1e-7; main.py's own inits bit for bit; nine tensors in each lr file; write ratios at init equal `ftb3b`'s per block). fp16 head-room probe of the init: peak 626 in the block-11 stream (1.0 % of the limit) against 134 for `ftbrhoplv`, finite in fp16 at init -> bf16 as planned. The 2-rank smokes in the same job hung at the first NCCL collective on dlc2gpu32 (the same hang as dlc2gpu03/08/11/17 this afternoon; the 1-GPU dump on the same node was fine); the two-epoch smoke + resume-check job (`resume_3b.sbatch`; its first submission failed on shell quoting of empty-valued flags, fixed) was cancelled for landing on the same node; a transport probe (`nccl_probe.sbatch`, job 29760822, on dlc2gpu32) found the cause: the two allocated GPUs sit on different PCIe host bridges (topology NODE) and NCCL's P2P transport hangs there, with or without SHM; `NCCL_P2P_DISABLE=1` makes the all-reduce complete in 0.3 s. The smoke + resume-check job was resubmitted with that setting (29760886: failed at once because `utils.cosine_scheduler` asserts warm-up < epochs, so a 2-epoch smoke cannot carry the real 50-epoch warm-up -> compressed schedule, 2 warm-up epochs to the real peak lr; 29760948 on dlc2gpu13: trained epoch 0 to step 370 and hung mid-epoch, SIGTERM-proof, until the job's time limit, P2P disabled or not; 29763105 pinned to dlc2gpu05, `timeout -k`): **PASS for both arms** -- three `[row-lr]` and six `[lr-scale]` lines, finite loss through the ramp to lr 2e-3 (`ftb3bpl` 7.20 -> 6.94 with the x331 fc2 group), checkpoints carry the lr specification, resume with the same file continues at epoch 2, a 1 % change in the file is refused with `lr specification mismatch on resume`, resume-safety check ok. Everything the brief asks for before a launch is done; NOT launched (waiting for Simon's go: `ftb3bpl` first, then `ftb3bb`, `ftb3bsl` last; 4 shared H200 + continuation, or 8 L40S with the hand-over). Test-node lesson: 2-rank smokes need `NCCL_P2P_DISABLE=1` AND a node that does not stall (dlc2gpu05 ok; 03/08/11/17/32 hang at the first collective without it, 13 stalled mid-epoch with it).

**REVERSE FREEZE `r0frzl` STARTED 2026-09-22 22:38 on the shared H200 partition (dlc2gpu22, job 29760130; the watcher released the held jobs `ftbck7p`/`ftbc7p`/`r0sup` with nice 5/10/20). Live check after epoch 0: blocks 8-11 identical to the timm init in 48 of 48 tensors, all 104 tensors outside them moved; the `[release]` lines show lr factor 0 for blocks 8-11 (0 before epoch 30, 1 from 50). Final Wed ~21:00.** Queued 17:00 as: random init, blocks 8-11 frozen for epochs 0-29, released 30-49 (results id 29760130, group hand-over 29760132). Verified by init dump (== r0) and the r0frz-verified freeze code; the 2-rank smoke hung at the first NCCL collective on three L40S test nodes today (08, 11, 17), so the freeze is checked on the live run's epoch-0 checkpoint. `ftbck7p`, `ftbc7p`, `r0sup` held until it starts (next shared slot ~21:10). Plan section 5e.

**FINAL `r0frz` (random + blocks 0-7 frozen for 30 epochs, released over 20) 79.16 (2026-09-22 15:44, last epoch; train loss 2.768).** +1.74 over `r0` (77.42), +1.08 over the old random mean (78.08); C kdyck 80.37. A pure learning-order manipulation with no checkpoint statistics gives half to three quarters of C's gain (n = 1); lens-7 at chance throughout, class formed in blocks 10-11, fit deficit larger than any winner's. `ftb4kc1` took the freed group slot at 15:45 as planned; `ftbc7p` took the freed shared slot (epoch 50). Details: `docs/early_lever_mechanism_plan.md` section 5e.

**COMPOSE ARM `ftbc7l` LAUNCHED (2026-09-22 14:35, group H200, dlc2gpu19; results id 29756597, hand-over job 29756600, continuation 29756601).** Committed kdyck early lever C (blocks 0-7, spec unchanged) + the late lever with the checkpoint's write statistics on v, proj and fc2 of blocks 9-11 (`profile_ftbrhoplv.json`'s multipliers = the `ftbrho` init; slow steps, no lr file), block 8 timm, fp16, seed 0. A first version with `ftbrhop`'s proj / fc2-only split (results id 29756513, launched 14:25) was cancelled at epoch ~3 on Simon's call and its directory set aside as `results_IMNET_BASE_29756513_superseded_projfc2only`. Verified (`plots/verify/verify_compose.py`: early part bit-identical to C, late multipliers exact, write ratios 1.4-1.6 in blocks 9-11, smoke on H200 after two NCCL hangs on L40S test nodes); reconstruction figures `plots/out/reconstruction_ftbc7l{,_scales,_late}.png` for Simon's check. Design and reading rules: `docs/early_lever_mechanism_plan.md` section 5g. ksd compose and the seeds (compose x 3, C x 2) are not launched.

**13:00-13:10 (2026-09-22): group L40S capped at 4 of our jobs (Simon), holds against the requeue trap, tiered hand-over wrapper.** Another wave (11:39-12:16) had preempted `ftbc7c1`, `ftb4kc1`, `ftb4c1` again and my own hand-overs had cancelled the queued group hand-overs and L40S fallbacks of the same arms on start. Fixes: (i) `handover_to_h200.sh` now knows route tiers (group H200 3 > shared H200 2 > group L40S 1 > shared L40S 0): a hand-over takes over from running jobs and from pending jobs on tiers <= its own, keeps pending upgrades on higher tiers, and exits untouched if the run already runs on a higher tier; (ii) the six lower-priority H200 jobs (`ftbck7a3f`, `ftbck7p`, `ftbc7a2`, `ftbc7p`, `r0sup`, `ftbck7g`) are HELD, so freed shared slots can only go to the intervention arms (`scontrol release` when those run); (iii) group L40S reduced to `ftbc7s`, `ftbc7ps`, `ftbc7g`, `ftbck7a1g` (the cancelled group jobs of `ftbck7a3qk`, `ftbck7a3f`, `ftbc7a2`, `ftbck7g` were resubmitted on the SHARED L40S partition, which had six idle nodes); (iv) `ftbck7s`'s shared-H200 job requeued, `ftbc7c1` took the slot (dlc2gpu25); (v) `ftb4c1` and `ftb4kc1` run on shared L40S meanwhile (dlc2gpu17 / 16) with their shared-H200 jobs first in that queue and group-H200 hand-overs chained onto `r0frz` (~15:50) and `ftbck7c1` (~21:15). Finals: `ftbck7ps` (P + S ksd) 79.04 at 12:16.

**FINAL `ftbck7a1` (committed ksd init, steps compensated) 79.59 (2026-09-22 10:40, last epoch; finished on group L40S after the hand-over): C ksd 79.90, -0.31, inside the seed resolution, no loss spikes: slow steps are not the source of the gain on ksd either.** `r0sup` is kept as a reference arm on Simon's decision (review verdict: not a causal test). 10:45-11:00: another shared-H200 preemption wave took `ftb4c1` (epoch 10), `ftb4kc1` (33) and `r0sup` (94); `ftbc7c1` and `ftbck7s` got the slots. 11:05: pending shared jobs re-ordered (the two full-checkpoint arms first, `ftbck7p` / `ftbc7p` / `r0sup` / `ftbck7g` to nice 300+), `ftbck7s` requeued to hand its slot to a full-checkpoint arm (its L40S hand-over takes the 12:20 L40S slot), `ftbc7c1`'s L40S hand-over cancelled (it would have pulled the run off H200), an L40S fallback hand-over added for `ftb4c1`. Group H200 still has room for 3 GPUs only (another lmb user holds 2 single-GPU jobs with ~21 h left), so the group moves stay chained to `r0frz` (~15:50) and `ftbck7c1` (~21:15).

**Churn between 09:50 and 10:35 (2026-09-22).** `r0frz`'s hand-over job hit its 23.5-h limit at 09:52 (epoch ~227); its continuation 29751229 resumed at once on the same group node (few minutes lost; ETA ~15:50). A shared-H200 preemption wave took `ftb4c1` (epoch 9), `ftbc7c1` (78), `ftbck7s` (~150), `ftbck7p`, `ftbc7p`; in their 2-min ineligibility window the freed slot went to `ftbck7g`'s H200 hand-over (which then cancelled its own group-L40S hand-over). 10:32: `ftbck7g` requeued by hand, `ftb4c1` took its slot (dlc2gpu25, resumed from the epoch-9 checkpoint); group-L40S hand-overs added for `ftbc7c1` (nice 0, takes the slot `ftbck7a1` frees ~10:45), `ftbck7s` (nice 10, the slot `ftbck7ps` frees ~12:20) and `ftbck7g` (nice 100); their shared-H200 jobs stay queued and win the run back when a shared slot frees. `r0sup` kept its slot (epoch 92; user priority, weak test per the review).

**Code review of the three intervention arms (2026-09-22, three reviewers; plan section 5f).** Freeze/release and the align loss are correct (bit-exact freeze verified on the checkpoints; lens == eval lens; DDP / AMP / accumulation / resume fine); one logging inconsistency fixed in engine.py (batch-wise wandb loss now excludes the auxiliary term). The suppress penalty `r0sup` is implemented as specified but is a logit-variance penalty with an LN-dilution loophole and 0.4% gradient share: not a test of causality; replacement (adversarial probe with gradient reversal) recommended, decision pending.

**Ten pending runs queued on the group L40S partition (2026-09-22 08:25, Simon's suggestion).** The group L40S partition had room for five of our 8-GPU jobs (cap 47 GPUs / 754 CPUs, other lmb users hold 5 GPUs) and seven usable nodes were idle. Hand-over jobs (`handover_to_h200.sh` with the partition overridden; they cancel the arm's H200 jobs on start) + continuations for `ftbck7a1` (epoch 285), `ftbck7ps` (275), `ftbc7s` (139), `ftbc7ps` (176), `ftbc7g` (139) started at once on dlc2gpu14 / 17 / 34 / 26 / 27 (8 ranks, resumed from the last checkpoints); `ftbck7g`, `ftbck7a1g`, `ftbck7a3qk`, `ftbck7a3f`, `ftbc7a2` wait on the account CPU cap and start as those finish (the two near-finished ksd runs free their slots at ~10:45 / 12:20); they also keep their H200 hand-overs, whichever starts first wins. `ftbck7p` and `ftbc7p` stay H200-only at nice 0 (250 epochs left: 40 h on L40S). Simulated finish times (no preemption): the last run (`ftbc7p`) Wed ~14:30; the full-checkpoint C1 arms Wed ~06:20.

**Both full-checkpoint C1 arms MOVED to shared H200 (2026-09-22 08:04-08:05, Simon: instead of less interesting runs).** The two gate-alone cells `ftbc7g` (epoch 139) and `ftbck7g` (epoch 127) were requeued right after a checkpoint (their lens is random-like and the MLP write does not hold, the least informative running pair) and `ftbck7a1`, `ftbck7ps`, `ftbc7s` pushed to nice 100; the freed slots went to `ftb4kc1` (dlc2gpu20, from scratch) and `ftb4c1` (dlc2gpu19, resumed from the L40S epoch-0 checkpoint); `ftbc7c1` took `ftbc7pg`'s slot (dlc2gpu22). BUG found and fixed: the generalised hand-over wrapper matched other jobs by their exported `SLURM_ID` and therefore missed the L40S PRIMARY (whose job id IS the results id): both L40S primaries kept training into the same directories for ~4 min until cancelled by hand at 08:08. `ftb4c1` is consistent (L40S wrote epoch 0, H200 resumed from it, L40S was stopped inside epoch 1 before any further write). `ftb4kc1` carries a cosmetic duplicate: the L40S job wrote an epoch-0 checkpoint and log line at 08:07:46 that the H200 job's own epoch 0 overwrote / appended to (log.txt has two epoch-0 entries, the second is the trajectory that continues; one extra wandb run). The wrapper now also cancels the primary. The group-H200 hand-overs (cancelled by the shared ones, as designed) were re-created: `ftb4kc1` after `r0frz` on dlc2gpu18 (~15:45), `ftb4c1` after `ftbck7c1` on dlc2gpu21 (~21:15). **FINAL `ftbc7pg` (profile + mean gate, kdyck) 79.92** (last epoch; C 80.37, P + S kdyck still running).

**C1 on the FULL procedural checkpoints LAUNCHED (2026-09-22 07:55 / 07:58, Simon's request, high priority, group H200 when free).** `ftb4c1` (kdyck4 checkpoint in all 12 blocks + auxiliary block-7 lens loss, w 0.3, epochs 0-49; results id 29754117) and `ftb4kc1` (ksd checkpoint; 29754123): verified (init == checkpoint bit for bit in all blocks, rest timm seed 0; smoke with aux window), started on idle shared L40S nodes, interim shared-H200 hand-overs at nice 0, group-H200 hand-overs chained onto our protected slots (`ftb4kc1` after `r0frz` ~15:45 on dlc2gpu18, `ftb4c1` after `ftbck7c1` ~21:15 on dlc2gpu21; the group cap has room for 3 GPUs only, another lmb user holds 2). Continuations on every job. References: ksd `ftb4` 80.21 (n = 1); kdyck `p` 29377576 = the kdyck4 checkpoint in all 12 blocks, seeds 0-2: 80.17 / 80.14 / 79.96, mean 80.09 +- 0.11 (confirmed on wandb 2026-09-22 08:20: `initialize` = pr_6066174, no block restriction, stored args equal to `ftb4c1`'s except data path copy and explicit defaults; my earlier note that `p` used an older checkpoint was wrong, `ftb4kd` is not needed). Ids: `results/init_dumps/launched_wave2.txt`.

**Two intervention arms dropped out overnight: shared-memory churn (2026-09-21 16:20-17:15, found 2026-09-22 06:45).** `ftbc7c1` (hand-over 29751054, group partition, dlc2gpu22) and `r0sup` (29751152, shared, dlc2gpu25) each burnt their six in-job retries within 45 min: DataLoader workers died with `could not unlink the shared memory file /torch_...: No such file or directory`, i.e. /dev/shm files removed under the running job while another user's jobs on the same node were being preempted and restarted every 1-3 min (dlc2gpu25: eight preemptions 16:16-16:33). Not a NaN, not our code. `ftbc7c1` had no continuation job (the hand-over jobs of wave 2 had none) and sat idle from 17:15 at epoch 68 with 71.24 at 64; `r0sup`'s continuation 29751230 waited in the shared queue (epoch 69, 73.86). Fixed 06:50: `ftbc7c1` resubmitted (29753656, continuation 29753657, `MAX_RETRIES=12`), a second continuation behind `r0sup`'s (29753658), continuations for the five other wave-2 H200 jobs (29753659-29753663), and every other pending shared-H200 job pushed to nice 610-800 so the two intervention arms take the slots that open at ~08:03 / 08:39 / 09:28 (`ftbc7pg`, `ftbck7a1`, `ftbck7ps` finishing). Rule: every H200 job gets a continuation; in-job retries do not survive an hour-long storm.

**FINAL `ftbc7a1` (committed kdyck init, steps compensated) 80.01 (2026-09-21 15:56, last epoch):** C 80.37, so -0.36, inside the seed resolution, despite nine loss spikes. Slow q, k steps are not the source of the gain. Its protected group slot went to `ftbck7c1` (hand-over 29751069, from scratch under results id 29751067). `ftbc7c1` recovered from its epoch-49/50 spike within three epochs (68.79 at epoch 54, C 68.32).

**Preemption wave on the shared H200 partition, 2026-09-21 14:51-15:33:** `ftbck7a1` (epoch 187), `ftbck7ps` (178), `ftbc7pg` (195), `ftbck7a1g` (182), `ftbck7a3f` (168), `ftbck7p` (56), `ftbc7p` (46) requeued; the four single-component cells took slots meanwhile (their hand-overs had the top priority). Pending jobs re-ordered by epochs left (closest to the end first). Simon: leave every run as it is, no pausing. First readings of the intervention arms: plan section 5e.

**WAVE-1 FINALS, first three (2026-09-21, last epoch): `ftbc7sg` (sink + gate only, kdyck, fp16) 79.57; `ftbck7sgb` (sink + gate only, ksd, bf16) 79.01; `ftbck7pg` (profile + gate, ksd) 80.27.** Against C 80.37 / 79.90 and random 78.08 (old) / 77.42 (`r0`): structure alone gives +1.5 / +0.9 over the old random, the profile adds ~0.85 (+- 0.28 over both tasks), the ksd sink is not needed given profile + gate. All three completed in one job (one DataLoader-worker retry in `ftbc7sg`); no-op continuations cancelled. 10:44-11:20 every shared-L40S job of ours was preempted by another group's array; the former screens there were handed over to H200 automatically (`ftbc7pg`, `ftbck7a1g`, `ftbck7a3qk`, `ftbck7a3f` now on shared H200), the wave-2 cells `ftbc7s`, `ftbc7g`, `ftbck7s` (epochs 9-10) wait; the group L40S cap (47 GPUs per account) is exhausted by the group.

**Priorities (Simon, 2026-09-21 10:30): the freeze arm and the two lens-loss arms first, on the group H200 partition where possible.** `r0frz`: its hand-over job 29751167 switched to the group partition (nodes of our own preemptable runs excluded) and started at once on dlc2gpu18, from scratch under results id 29751165 (the wrapper cancelled the pending L40S jobs; first end-to-end use of its no-checkpoint path); continuation 29751229 (`r0sup`: 29751230). `r0sup`: hand-over 29751152 at nice 0, first in line for the shared slot that opens when `ftbc7sg` ends (~11:40). `ftbc7c1` / `ftbck7c1`: hand-overs 29751054 / 29751069 inherit our protected group slots when `ftbck7sgb` (~12:20) and `ftbc7a1` (~15:55) finish (dependency + same node, no preemption). `ftbck7p` and the `ftbc7ps` hand-over went back to the shared queue. Earlier the same morning: Phase 0 completed with `r0` (job 29751186; plan section 1.8).

**Both converse arms of C1 LAUNCHED (2026-09-21 09:45 / 09:52, Simon's go).** `r0sup` 29751150: plain random + suppress penalty on the block-7 lens (weight 0.3, epochs 0-49). `r0frz` 29751165: plain random, blocks 0-7 frozen for epochs 0-29, linear release over 30-49 (new `--release_blocks/_start/_end`, verified in task 29751149: bit-exact freeze, release moves all 96 tensors). Both wait on the shared-L40S CPU cap with H200 hand-over jobs queued (29751152, 29751167). Random's block-7 lens peaks at epochs 29-39 (47.8-49.3%) and decays to 15-25% by the end; winners peak at 14-19 below 18% and decay to ~0.

**Wave 2 LAUNCHED (2026-09-21 09:30): sink alone / gate alone on timm for both tasks (`ftbc7s` 29751055, `ftbc7g` 29751058, `ftbck7s` 29751061, `ftbck7g` 29751064) and C1 = committed init + auxiliary block-7 lens cross-entropy, weight 0.3, epochs 0-49 (`ftbc7c1` 29751052, `ftbck7c1` 29751067).** Each starts on 8 L40S with an H200 hand-over job queued; verified (array 29750978 / 29751018). New code: `utils.AuxLensLoss` + `--aux_lens_*` (inert by default), `vitbase_runs/handover_to_h200.sh`, `plots/verify/verify_resume_checkpoint.py`, single-component support in `verify_structure_only.py`. The converse `r0sup` (suppress penalty on plain random) is verified and held. Table: `docs/early_lever_mechanism_plan.md` section 5e; ids: `results/init_dumps/launched_wave2.txt`. dlc2gpu04 has a GPU with uncorrectable ECC errors: excluded.

**Monday morning (2026-09-21): R0 final, preemption churn, protected slots, automatic hand-over.** `r0` 29744580 finished: **77.42** last epoch (old random n = 3: 78.08 +- 0.19, seeds 77.91 / 78.04 / 78.28; the stored launch arguments differ only in data path copy, worker count and `skip_norm`, which is inert without a checkpoint: 2.4 sigma low, seed or code-version noise, unresolved). Overnight ~14 preemptions of our shared-H200 jobs (`ftbc7sg` once, `ftbc7a1` three times); a requeued job comes back with priority ~700 for up to 20 min, so the freed slot goes to whatever else of ours is pending. 07:48 `ftbc7a1` (epoch 190) and 07:53 `ftbck7sgb` (epoch 241) moved to the group H200 partition (not preemptable; hold / partition switch / ReqNodeList on their own node; `ftbck7sgb` resumed in bf16 after ~2 min). Side effect: `ftbck7p` (epoch 55) was preempted on dlc2gpu22 when the group job needed the CPUs a newcomer had taken. `ftbc7sg` stays on the shared partition (move interrupted by Simon). Hand-over jobs 29750268-29750273 (`vitbase_runs/handover_to_h200.sh` + `plots/verify/verify_resume_checkpoint.py`, unit-tested) wait on the shared H200 partition for the six L40S runs: when one starts it cancels the L40S twin, checks the checkpoint and resumes. `ftbck7sgb`'s one failed attempt (05:01, epoch 205) was a DataLoader worker death, not a NaN.

**`ftbc7a1g` COLLAPSED and was paused (2026-09-20 19:50).** Train loss 4.16 (epoch 40) -> 5.09 -> 6.90 (epoch 42, chance), test accuracy 1.38 at epoch 44, no recovery by epoch 48; no NaN, so no alarm fired. Continuation 29748442 cancelled (resumable from checkpoint-48; per-epoch model checkpoints 39-48 cover the collapse); `ftbc7a2`'s continuation 29748444 took the protected group-L40S slot. Loss spikes are specific to the kdyck P + S + G init and scale with the q, k step size (x 0.5: one, C: four moderate, x m: many, to chance level): table in `docs/early_lever_mechanism_plan.md` section 5b.

**All seven screens CONTINUE past epoch 39 (2026-09-20 17:55, Simon: keep everything running on L40S overnight, move to H200 tomorrow).** Continuations 29748440-29748446 (`resume_zero_kbias.sh`, `STOP_AFTER_EPOCH=-1`, afterany on the screen jobs, 8 L40S, 23.5 h; four on the group L40S partition, three on the shared one). `ftbck7a3qk` was preempted on the shared L40S partition at 17:53 (epoch 38; requeued, its continuation follows). Readings at epoch 29-49: `docs/early_lever_mechanism_plan.md` section 5b.

**Two wave-1 runs PREEMPTED on the shared H200 partition (2026-09-20 17:18).** `ftbc7a1` 29745240 (epoch 58, dlc2gpu20) and `ftbck7a1` 29745242 (epoch 56, dlc2gpu25) were requeued by higher-tier group-partition jobs (`preempt/partition_prio`, mode REQUEUE). A requeued job is only eligible again after ~2 min, so 28 s later the freed GPUs on dlc2gpu25 went to the LOWEST tier of our queue, `ftbc7p` 29745252. Put back in order at 17:40: `ftbc7p` requeued by hand (it had reached epoch 4), `ftbc7a1` took the slot and resumed from checkpoint-58 with its row mask (lr spec validated). `ftbck7a1` waits for the next slot. `ftbck7pg` 29745246 had started at 15:33 (dlc2gpu24). The structure-only runs are on the same preemptable partition; the group H200 cap is full until R0 ends (Mon ~07:30).

**Per-epoch checkpoints THINNED (2026-09-20 ~14:00, Simon's go).** 23,885 `checkpoint-<E>-model.pth` files (4.14 TB) of 108 finished seed directories deleted with `plots/verify/thin_epoch_checkpoints.py` (validated manifest + deletion log in `results/init_dumps/`); workspace 9.85 -> 5.80 TB. Kept: 0, 1, 2, 4, 5, 9, 10, 14, 19, 20, 29, 39, 49, 50, 59, 99, 100, 199, 200, 299, every 25th in both conventions, the last epoch present; nine reference arms keep the wave-1 set (0-59 + every 10th). Untouched: full checkpoints, best, logs; all running / queued runs; 17 seed directories referenced by the collaborator's `kempfe/dynamics` analysis and the seven ksd `ftb4*` runs owned by the collaborator's account; the two late-lever runs that had just finished. Details: `docs/early_lever_mechanism_plan.md` section 5c.

**Structure-only cells STARTED; ksd cell switched to bf16 (2026-09-20).** `ftbck7sg` 29745248 (12:10) and `ftbc7sg` 29745254 (12:22) started on the slots freed by the late-lever runs. `ftbck7sg` (fp16) lost a finite loss in epoch 9 (lr 3.9e-4, no warning) and then at the same iteration of epoch 10 on four resumes: dead after 6 attempts, continuation cancelled. Restarted from scratch in bf16 as `ftbck7sgb` 29746035 (continuation 29746036; 13:21, init bit-identical, hash `32e2e50a40b1b34f`). `ftbc7sg` continues in fp16 (clean through epoch 15); `run_train_ftbc7sgb.sh` is ready. Cause not localised: block 1-2 sink logits are 10x those of the committed arm but < 10% of the fp16 range on 51k augmented inputs, and 1.28 M inputs through the epoch-9 checkpoint under fp16 stay finite; one event was reproduced and dumped (crash-path dump `utils.dump_nonfinite_state`, new), autopsy deferred until it happens again. Details: `docs/early_lever_mechanism_plan.md` section 5b.

**v-inclusive late-lever FINALS (2026-09-20 ~12:20 / 12:56, last epoch): `ftbrhoplv` 80.15 (max 80.23), `ftbrhoslv` 78.18.** Twins without v: `ftbrhopl` 80.13, `ftbrhosl` 78.31, so v-inclusion moves nothing (+0.02 / -0.13, seed resolution 0.45); the late lever is the loud write, not the step size, also on `ftbrho`'s own init. Both completed in one job (exit code 0, post-training analysis written); continuations 29737717 / 29737719 cancelled as no-ops. Table: `docs/proc_init_recipe.md` section 13.

**Wave-1 queue re-ordered (2026-09-20 11:55): the structure-only cells first.** Simon: the runs without effective scales (sink + gate only) might be the most interesting. `ftbck7sg` 29745248 and `ftbc7sg` 29745254 (both full 300-epoch runs) and their continuations moved from nice 100 / 200 to nice 0, the two A1 runs from 0 to 50; Slurm's plan went from tonight 21:11 / Monday 12:19 to today ~12:48 / ~12:58 (the slots that open when the v-inclusive late-lever runs and another user's 24-h jobs end). fp16 as launched; bf16 remains the fallback if one diverges during the warmup.

**Wave-1 screens MOVED to L40S (2026-09-20 11:40).** The shared H200 nodes had no slot (no node with 4 free GPUs + 192 CPUs + 750 GB), so the seven 40-epoch screens were cancelled there (29745256-29745263) and resubmitted with 8 L40S each (update frequency 4: batch 4096 and schedule unchanged, `NCCL_SHM_DISABLE=1`, `--time 09:00:00`): group L40S partition (not preemptable) `ftbc7ps` 29745319, `ftbc7pg` 29745320, `ftbc7a1g` 29745321, `ftbck7a1g` 29745322; shared L40S partition (preemptable, requeue + auto-resume) `ftbc7a2` 29745327, `ftbck7a3qk` 29745324, `ftbck7a3f` 29745325 (the fifth group-partition job hit the account's CPU cap of 754). All seven started within two minutes; 0.457 s per micro-step = 9.5 min per epoch, epoch 39 after ~6.9 h. The eight full runs stay queued on the shared H200 partition. A promoted screen would continue on 4 H200 (same batch and schedule, not bit-comparable from the promotion epoch on).

**Mechanism study, wave 1 LAUNCHED (2026-09-20 ~11:15, shared partition, 8 full runs + 7 forty-epoch screens).** Step-size arms on the bit-identical committed init (`ftbc7a1` 29745240, `ftbck7a1` 29745242; screens `ftbc7a1g`, `ftbck7a1g`, `ftbc7a2`, `ftbck7a3qk`, `ftbck7a3f`) and the sink x gate x profile cells (`ftbck7ps` 29745244, `ftbck7pg` 29745246, `ftbck7sg` 29745248, `ftbck7p` 29745250, `ftbc7p` 29745252, `ftbc7sg` 29745254; screens `ftbc7ps`, `ftbc7pg`). All verified (array 29744815; structure-only arms additionally under the real warmup schedule, job 29744861). New in main.py: `--stop_after_epoch` (screens on the unchanged schedule) and `--analysis_ckpt_dense_until / --analysis_ckpt_every` (thinned per-epoch checkpoints; workspace at 9.15 TB of 10 TB). Table, multipliers and verification: `docs/early_lever_mechanism_plan.md` sections 5b-5c; ids in `results/init_dumps/launched_wave1.txt`.

**R0 LAUNCHED + Phase 0 of the mechanism study (2026-09-20 09:31).** `r0` = plain timm random, protocol of the committed arms (`run_train_r0.sh`), group partition, job 29744580 (continuation 29744581), seed 0; verified by job 29744579 (init == timm seed 0 bit for bit, 2-rank smoke). It is the random reference WITH per-epoch checkpoints that the original `r` lacks. Phase 0 (no training) ran on all 107 finished runs with per-epoch checkpoints: plan, evidence, figures and the revised experiment order are in `docs/early_lever_mechanism_plan.md` (sections 1.7 and 5), figures in `plots/out/phase0/figs/`.

**Mean-gate FINALS (2026-09-20 07:57-08:29, one seed each, last epoch): `ftbanapermb7i` 80.37 (kdyck, write side at timm; max 80.38), `ftbanakpermb7i` 79.90 (ksd, write side at timm; max 79.90), `ftbanakpermb7` 79.54 (ksd, checkpoint write scales; max 79.60).** Every mean-gate arm finishes above its active-unit twin: +0.74 / +0.38 on kdyck (timm / checkpoint write side), +0.25 / +0.90 on ksd; block-7 lens peaks 9.2 (19), 4.0 (14), 15.9 (19), lower than the twins' in every pair. Gate target DECIDED: mean pre-activation, the same on both tasks. Committed design = q, k, fc1 effective scales + LayerNorm statistics + entropy targets + mean gate, v / proj / fc2 at timm: 80.37 on kdyck (prefix 79.89, n = 3), 79.90 on ksd (prefix 80.05, n = 1). The mean gate is also the only target under which the structure-only cell is reachable on a timm init (proc_init_recipe.md section 12). `ftbanapermb7i` ran its first 38 epochs on the shared partition (29737095) and the rest on the group partition (29737711), same results directory.

**`ftbanapermb7w` FINAL 78.91 (2026-09-19 14:45, last epoch = max):** kdyck, blocks 0-7 recipe with the mean-pre-activation gate and v / proj / fc2 write-matched to the prefix's ratios. Identical to `ftbanaperab7w` (active-unit gate, 78.90): on kdyck the write-matched arms sit at 78.9 whatever the gate target, 1.2 below the best arm (`ftbanapermb7` 80.12) and 0.7-0.8 below the arms that leave the write side at timm or at the checkpoint's effective scales. Write-matching is settled as harmful on both tasks; the gate target is settled as irrelevant on kdyck. Continuation 29729557 runs the corrected post-training analysis.

**`ftbanaperab7i` FINAL 79.63 (2026-09-19 13:15, last epoch; max 79.66):** kdyck, blocks 0-7 recipe with q/k/fc1 effective scales + LayerNorm statistics + entropy targets + active-unit gate, v / proj / fc2 at timm. 0.26 below the 0-7 prefix (79.89, n = 3) and 0.11 below `ftbanaperab7` (checkpoint effective scales on the write side, 79.74): inside the seed resolution, so the write side at timm is confirmed on kdyck as it was on ksd (`ftbanakperab7i` 79.65). Its continuation job (29733657) ran the corrected post-training analysis: the reloaded checkpoint-299 evaluates to 79.62, and the accuracy JSON's top-level fields of this run are now right. Run then moved off the group partition: `ftbanapermb7i` resumed there as 29737711 from checkpoint-38 (shared-partition pair 29737095 / 29737096 cancelled at 13:18).

**`ftbrhoplv` / `ftbrhoslv` LAUNCHED (2026-09-19 ~12:35, alldlc, ids 29737716 / 29737718, continuations 29737717 / 29737719):** the v-inclusive late-lever step-size cells (proc_init_recipe.md section 13). `ftbrhoplv` = `ftbrho`'s init (v = proj x3.0071 / 4.6048 / 7.6129, fc2 x9.4186 / 27.4658 / 81.7061 in blocks 9-11, `profile_ftbrhoplv.json` key `extra`) with lr x the same multipliers on proj, fc2 and, through the new per-row lr mask (`row_lr_mask.py`), on the v rows of the fused qkv; bf16 as `ftbrhopl`. `ftbrhoslv` = timm init with the reciprocals; fp16 as `ftbrhosl`. `ftbrho` (n = 3) is the loud + slow cell. Verified: init dumps through main.py bit-identical to the reconstruction / to timm, ftbrho dump matched to 6.5e-6, write ratios at init equal ftbrho's, lr files exact, 2-rank smokes with checkpoint lr-spec check (`plots/verify/verify_late_trio_v.py`, jobs 29737705 / 29737710). Also today: external code review answered (section 15 of the recipe doc; the post-training reload dropped head/embeddings -- accuracy JSON top-level fields are ~0.1% for earlier runs, nothing reported uses them); verifiers moved to the GPU (40 min -> 2 min per arm).

**`ftbanakperab7w` FINAL 78.09 (2026-09-19 ~09:10, last epoch; max 78.16):** ksd, v / proj / fc2 write-matched -- random level
(+0.01), as its block-7 lens peak of 37.7 had suggested; the write-matched arms are now 78.90 / ~78.9 (kdyck) and 78.09 (ksd).

**Overnight finals of the blocks-0-7 family (2026-09-19 03:24-08:00, one seed each, last epoch).** `ftbanapermb7` (kdyck, the
mean-pre-activation gate instead of the active-unit gate, otherwise `ftbanaperab7`) **80.12** (max 80.12): +2.04, 0.23 ABOVE the
0-7 prefix `ftb4i` (79.89, n=3) and 0.38 above `ftbanaperab7` (79.74) -- inside the seed resolution of both, so the gate target
does not separate on kdyck; both weights-only recipes sit at the prefix. `ftbanakperab7i` (ksd, sink + active-unit gate on the
input-side scales, v / proj / fc2 left at timm's) **79.65** (max 79.69): +1.57, level with the bias-based sink arm `ftbanaks`
(79.58), 0.4 below the ksd prefix (80.05, n=1) and 0.27 below `ftbanaksg` -- inside the seed resolution; a full point above
`ftbanakperab7` (78.64), whose only difference is v / proj / fc2 at the checkpoint's effective scales. `ftbanaperab7w` (kdyck,
v / proj / fc2 write-matched) **78.90** (max 78.93): +0.82, sub-winner, as its block-7 lens peak of 33 had suggested.
`ftbanakbs` (ksd, persistent fc1 bias gate, no sink; ten preemptions) **78.27**: random level, closing the ksd 2x2 -- the gate
alone does nothing with or without persistence, the sink carries the effect. Still running: `ftbanaperab7i` (kdyck, 79.2 at 234),
`ftbanapermb7w`, `ftbanakperab7w` (78.06 at 289).

**First finals of the blocks-0-7 family (2026-09-18, one seed each, last epoch).** `ftbanaperab7` (kdyck: exact scales of all six
weights + LN statistics + entropy component + active-unit gate, 5-seed targets, exact realisation) **79.74** (max 79.74): +1.66
over random, 0.15 below the kdyck 0-7 prefix `ftb4i` (79.89, n=3) -- inside the seed resolution; its block-7 logit lens peaked
at 18 (earlier winners 25-30, prefix 7). `ftbanakperab7` (ksd, same recipe) **78.64** (max 78.74): +0.56, a point below the
bias-based sink arm `ftbanaks` (79.58) and 1.4 below `ftbanaksg` / the ksd prefix: the weights-only sink + gate give only part
of the ksd effect; the arm was flat from epoch 209 on. Still running at the time: `ftbanakperab7i` 79.48 at epoch 244 (the ksd
arm with v/proj/fc2 left at timm's, lens peak 7.3 = prefix), `ftbanaperab7w` flat at 78.6-78.7 (write-matched), `ftbanaperab7i`
76.5 at 109; `ftbanakbs` 78.25 at 284 (persistent gate without sink, random level, ten preemptions).

**Launched 2026-09-17 20:22-20:45, one seed each, `alldlc2_gpu-h200` (the group partition's 13-GPU account quota was held by two
colleagues' 24 h jobs, so nothing could start there), each with a `resume_zero_kbias.sh` continuation.** The blocks-0-7 recipe
family (exact scales + LN statistics + attention-entropy component + fc1 gate, 5-seed targets, exact realisation; statement
and init signatures in `docs/proc_init_recipe.md` section 9): ksd `ftbanakperab7` 29729543, `ftbanakperab7w` 29729547,
`ftbanakperab7i`; kdyck `ftbanaperab7` 29729545, `ftbanaperab7w` 29729541, `ftbanaperab7i`; kdyck controls with the gate
calibrated to the mean pre-activation instead of the active-unit fraction: `ftbanapermb7` 29729553, `ftbanapermb7w`.
Results ids = these job ids. `w` = v, proj, fc2 write-matched with the late lever's sqrt split; the `vw` variants are the same
function at init and were not launched. All arms passed both verifiers, dumps through `main.py` and a two-rank fp16 smoke run.
Finals that landed today: `ftbanapx` 79.18 (exact folding, q/k separate, ramp; last epoch), `ftbanakx` 77.68 (ksd exact scales).

**`ftbanape` FINAL 79.44 (2026-09-17 16:26, one seed, last epoch; max 79.44).** The least-processed kdyck recipe (exact
per-block effective scales with rms(W diag gamma) folding, q and k separate, no ramp, no fc2 correction, 72 inline LN
statistics, blocks 0-8): +1.36 over random, 0.55 below the 0-8 prefix `ftb3i` (79.99, n=3) and 0.80 below `ftbanap` (80.24,
n=1: product folding, q/k pooled to 1.32, fc2 end 0.95, ramp). `ftbanapx` (exact folding, q/k separate, ramp) is at 78.96 at
epoch 269 (`ftbanape` moved +0.10 from epoch 265 to the end). Both clean variants sit below the corrected recipe by about two
to three sd of a single-seed difference (0.39); whether that is the q/k convention, the folding or seed noise is open: seed
repeats of `ftbanap` / `ftbanape` and an exact arm with `--query_key pooled` would tell. No-op continuation 29723599 cancelled.
Precedents for the 0-7 sufficiency arms, from the rho series: `ftb4m` (checkpoint 0-7 with v / proj / fc2 downscaled to the
random init's write ratios, 8-11 random) 80.00 = the prefix, so the write profile is not needed when the weights are the
checkpoint's; `ftb4o` (random 0-7 write-matched to proc's ratios, nothing else) 77.27 = -0.80, and on ksd `ftbanakw`
(recipe + fitted write profile) 76.65: loud early writes with random content have been harmful twice.

**`ftbanaksg` FINAL 79.92 (2026-09-17 13:45, one seed, last epoch; max 79.95).** ksd second-moment recipe + attention sink +
persistent fc1 gate (blocks 0-8): +1.84 over random (78.08 +/- 0.19), 0.13 below the ksd prefix `ftb4i` (80.05, n=1), i.e.
inside the seed resolution of 0.45. Sink alone (`ftbanaks`) 79.58, recipe alone (`ftbanak`) 77.86. Preempted six times on
the shared partition, resumed from epoch checkpoints each time; no-op continuation 29720337 cancelled.

**Input side by scale, output side by write ratio: three sufficiency arms on blocks 0-7 (2026-09-17; set up and verified, not
launched).** The early lever takes weights-only effective scales from the checkpoint, the late lever a write ratio. Measured
reason for the difference: the kdyck tail's own weights write 0.26 / 0.13 of their own stream (rms 52) and 1.45 / 4.85 of a
random prefix's (rms 1.4), so a scale does not carry over for proj / fc2, which write into the un-normalised stream, while it
does for q, k, v, fc1 behind a LayerNorm. Arms (kdyck / ksd; statement, targets and table in `docs/proc_init_recipe.md`
section 8): (1) `ftbanapeb7i` / `ftbanakpeb7i` = q, k, fc1 scales + LN statistics, v / proj / fc2 timm's; (2) `...b7w` = (1) +
v, proj, fc2 write-matched to the prefix's per-block ratios as the late lever does (sqrt split over v and proj);
(3) `...b7vw` = (1) + v by effective scale + proj, fc2 write-matched. New code: `utils.sublayer_write_ratios`,
`utils._match_write_ratio`, spec key `write_ratio` in `utils.calibrate_joint_statistics` (refuses fc2 write ratio + common
write in one block), `main.py` call site (broadcast of qkv / proj / fc2, `[write_ratio]` log lines), `extract_profile.py
--scale_weights / --write_ratio`, both verifiers, `plot_reconstruction.py` (twelve panels: + stream rms, token-specific MLP
write). All six PASS both verifiers; through `main.py` (job 29728161) the dumps differ from each other only in the intended
tensors, which are exact scalar multiples; two-rank fp16 smoke runs train. Computed behaviour (kdyck): (2) and (3) are the
same function at init (12 quantities agree to three decimals) and reproduce the write ratios, the stream size (53.8 vs 53.0
entering block 8; (1) 1.13, base 2.9) and the quiet random tail (0.5% / 0.7%), token cosine 0.74 (prefix 0.915, (1) 0.40);
no sink, no gate. Cost: proj x19-27 (or v, proj x5.3-6.7 each), fc2 x4-13 and x63-66 in block 0, so the output side is also
slow (confound, as in `ftbrhop`). Earlier necessity result: e-series, full checkpoint with v / proj / fc2 rescaled to the
random init's write ratios, 80.21.
