...
def main(args):
    ...
    vit_small_probe_dims = {
        ...
        "token-wise": 384,  # for probing each token separately with a shared probe across tokens
    }
    ...
    stream_stats = StreamingAllProbeStats(
        feature_dims=[vit_small_probe_dims[args.probe_mode]] * (2 * args.num_blocks), # for each block, we collect stats on both the attention output and the block output, so multiply num_blocks by 2
        num_classes=args.nb_classes,
        device=stats_device,
    )

    start_epoch, start_batch_idx = load_streaming_stats_checkpoint(stream_stats, args.output_dir)
    ...
    for epoch in range(1):
        ...
        for batch_idx, (images, targets) in enumerate(data_loader_train):
            ...
            if args.output_dir and ((batch_idx + 1) % 20 == 0):
        ...
        save_streaming_stats_checkpoint(
            ...
            master_epoch=master_epoch,
            epoch=epoch,
            batch_idx=len(data_loader_train),
            rank=rank,
        )
    ...
    metrics = stream_stats.finalize(eps=1e-5)
