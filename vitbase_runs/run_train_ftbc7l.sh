...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --notes "COMPOSE arm (docs/early_lever_mechanism_plan.md 5g): committed kdyck early lever C (blocks 0-7, spec of ftbanapermb7i unchanged) + late lever with the checkpoint write statistics on all three write matrices (blocks 9-11: v = proj x 3.0071/4.6048/7.6129, fc2 x 9.4186/27.4658/81.7061 = ftbrho init, as profile_ftbrhoplv.json), slow steps (no lr file), fp16; block 8 timm; seed 0" \
    ...
    --profile_spec "vitbase_runs/profile_ftbc7l.json" \
