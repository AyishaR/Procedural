...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --notes "least-processed second-moment recipe of the kdyck checkpoint: exact per-block effective scales (54 numbers, rms(W diag gamma) folding, q/k separate, no ramp, no fc2 correction) + 72 inline LayerNorm statistics (extract_profile.py --gain_fold exact --exact); blocks 0-8, random 9-11, no checkpoint at init" \
    ...
    --profile_spec "vitbase_runs/profile_ftbanape.json" \
