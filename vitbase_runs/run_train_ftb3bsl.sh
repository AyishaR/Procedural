...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --notes "late lever at proc MEASURED late-block profile, random write + slow steps (docs/late_lever_measured_profile_brief.md): timm init, lr x 1/multiplier on the v rows, proj, fc2 of blocks 9-11 with the ftb3b multipliers (proj, fc2: wd x multiplier; v rows: wd unchanged), exactly the ftbrhoslv construction; fp16" \
    ...
    --skip_norm true \
    --init_method "default" \
    --lr_scale_json "vitbase_runs/lrscale_ftb3bsl.json" \
