...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --notes "mechanism study wave 2 (docs/early_lever_mechanism_plan.md 5d): SINK ALONE: rank-one attention sink (entropy targets) on a plain timm init, no gate, no scales, no LayerNorm statistics; components add to the matrix (renormalize false); ksd, blocks 1-7; bf16 as ftbck7sgb" \
    ...
    --profile_spec "vitbase_runs/profile_ftbck7s.json" \
