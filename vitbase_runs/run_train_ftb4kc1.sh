...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --notes "mechanism study wave 2, C1 on the FULL procedural checkpoint (docs/early_lever_mechanism_plan.md): ksd checkpoint loaded into all 12 blocks (as ftb4 ksd 80.21) + auxiliary cross-entropy on the block-7 lens, weight 0.3, epochs 0-49, then off" \
