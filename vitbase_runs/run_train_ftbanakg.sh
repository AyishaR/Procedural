...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --notes "ksd analogue of ftbanag: timm random with ksd's exact per-block effective scales (54 numbers, q/k/v/fc1 divided by rms gamma) and ksd's LayerNorm gain and bias vectors permuted across channels, blocks 0-8; blocks 9-11 random, no other checkpoint content" \
    ...
    --profile_spec "vitbase_runs/profile_ftbanakg.json" \
