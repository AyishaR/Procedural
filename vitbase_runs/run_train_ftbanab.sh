...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --notes "analytic profile plus proc LayerNorm biases (permuted) in blocks 0-8: timm random, per-slice rms multipliers (block 0 + linear ramp 1-8) from vitbase_runs/profile_ftbanab.json, LN=1, biases 0, no checkpoint" \
    ...
    --profile_spec "vitbase_runs/profile_ftbanab.json" \
