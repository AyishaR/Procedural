...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --notes "R0: plain timm random init, nothing else; protocol identical to the committed arms (run_train_ftbanapermb7i.sh minus the init lines), code of 2026-09-19 (fixed post-training reload), per-epoch model checkpoints kept: the random reference for the mechanism study (docs/early_lever_mechanism_plan.md, R0). The original r (78.08 +- 0.19, n = 3) predates per-epoch checkpoints." \
