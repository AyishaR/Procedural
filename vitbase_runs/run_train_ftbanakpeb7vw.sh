...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --notes "ksd blocks 0-7 (reference ksd ftb4i 80.05), timm random + statistics of the checkpoint, no checkpoint at init; (3) = (1) + v by its exact effective scale + proj, fc2 write matched: proj takes the whole attention factor (--scale_weights q,k,v,fc1 --write_ratio proj,fc2)" \
    ...
    --profile_spec "vitbase_runs/profile_ftbanakpeb7vw.json" \
