...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --notes "mechanism study wave 2 (docs/early_lever_mechanism_plan.md 5d): GATE ALONE: rank-one mean gate (pre-activation mean targets) on a plain timm init, no sink, no scales, no LayerNorm statistics; components add to the matrix (renormalize false); kdyck, blocks 1-7; fp16 as ftbc7sg" \
    ...
    --profile_spec "vitbase_runs/profile_ftbc7g.json" \
