...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --notes "mechanism study wave 1 (docs/early_lever_mechanism_plan.md): committed init, lr x m (weight decay / m) on the q rows, k rows (row lr mask) and fc1 of blocks 0-7: random-init relative Adam steps; ksd, blocks 0-7, committed protocol" \
    ...
    --lr_scale_json "vitbase_runs/lrscale_ftbck7a1.json" \
