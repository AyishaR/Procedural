...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --notes "ftbanak (ksd 18 ramp numbers + 36 sampled LN statistics) plus a per-block attention sink in blocks 1-8: the q part of the qkv bias set to a seeded random unit direction per head with norm B_b (8 numbers) so that the attention entropy at init matches the ksd prefix per block; every query of a head reads one key (common-mode attention write), as both procedural prefixes do at init; blocks 9-11 random" \
    ...
    --profile_spec "vitbase_runs/profile_ftbanaks.json" \
