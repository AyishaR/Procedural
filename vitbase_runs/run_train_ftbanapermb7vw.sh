#!/bin/bash
#SBATCH --job-name ftbanapermb7vw
...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --notes "statistics-only recipe for kdyck blocks 0-7 (reference ftb4i 79.89), timm random + numbers read from the checkpoint, no checkpoint at init: exact effective scales + 64 LayerNorm statistics; q/k: rank-one sink calibrated to the prefix's attention entropy (blocks 1-7); fc1: rank-one gate calibrated to the prefix's MEAN fc1 PRE-ACTIVATION (blocks 1-7; control of the gate target: ftbanaperab7vw uses the active-unit fraction, which on kdyck is met with a mean of -0.7 and GELU rms 0.16 against the prefix's -2.4 and 0.03); no block-0 common write; calibration order per block: entropy, attention write, active fraction, MLP write; (3) v by its exact effective scale, proj and fc2 write-matched (--scale_weights q,k,v,fc1 --qk_entropy --fc1_gate --fc1_gate_target pre_activation_mean --write_ratio proj,fc2)" \
    ...
    --profile_spec "vitbase_runs/profile_ftbanapermb7vw.json" \
