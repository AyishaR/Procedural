...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --notes "reference for ftb4c1 (docs/early_lever_mechanism_plan.md): kdyck4 checkpoint loaded into all 12 blocks (head, cls, pos, patch embedding and final norm random), current protocol, no intervention" \
