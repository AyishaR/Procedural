...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --batch_size $BATCH_SIZE --lr 2e-3 --update_freq $UPDATE_FREQ --use_amp true --amp_dtype bfloat16 \
    ...
    --notes "late lever at proc MEASURED late-block profile, normal steps (docs/late_lever_measured_profile_brief.md): ftb3b init replayed from JSON (v = proj x sqrt(a_b), fc2 x f_b in blocks 9-11, multipliers read off ftb3b_s0.pth; targets attention 1.380/2.098/0.777, MLP 4.757/4.459/0.526) + lr x the same multipliers (proj, fc2: scalar groups with wd / multiplier; v rows via the row lr mask, wd unchanged), exactly the ftbrhoplv construction; bf16" \
    ...
    --profile_spec "vitbase_runs/profile_ftb3bpl.json" \
    --lr_scale_json "vitbase_runs/lrscale_ftb3bpl.json" \
    --init_method_scaled_blocks "" \
    --analysis_ckpt_dense_until 60 --analysis_ckpt_every 10 \
