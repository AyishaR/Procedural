...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --notes "timm random init with per-tensor lr / multiplier (wd x multiplier) on proj/fc2 of blocks 9-11, multipliers as ftbrhop: random-scale writes, ftbrhop's slow Adam relative steps (slow, not loud)" \
    ...
    --skip_norm true \
    --init_method "default" \
    --lr_scale_json "vitbase_runs/lrscale_ftbrhosl.json" \
