...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --notes "mechanism study wave 1 (docs/early_lever_mechanism_plan.md): committed init, lr x 0.5 on the q rows, k rows and fc1 of blocks 0-7: slower than the natural steps; kdyck, blocks 0-7, committed protocol; 40-epoch screen on the unchanged 300-epoch schedule" \
    ...
    --lr_scale_json "vitbase_runs/lrscale_ftbc7a2.json" \
