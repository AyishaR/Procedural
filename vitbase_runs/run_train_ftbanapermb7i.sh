...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --notes "statistics-only recipe for kdyck blocks 0-7 (reference ftb4i 79.89), timm random + numbers read from the checkpoint, no checkpoint at init: exact effective scales + 64 LayerNorm statistics; q/k: rank-one sink calibrated to the prefix's attention entropy (blocks 1-7); fc1: rank-one gate calibrated to the prefix's MEAN fc1 PRE-ACTIVATION (blocks 1-7; the gate target that gave 80.12 on kdyck as ftbanapermb7); no block-0 common write; calibration order per block: entropy, attention write, gate, MLP write; (1) v, proj, fc2 at timm's scale (--scale_weights q,k,fc1 --qk_entropy --fc1_gate --fc1_gate_target pre_activation_mean)" \
    ...
    --profile_spec "vitbase_runs/profile_ftbanapermb7i.json" \
