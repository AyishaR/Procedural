...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --notes "REVERSE FREEZE, control for r0frz (docs/early_lever_mechanism_plan.md): plain timm random init, blocks 8-11 at lr factor 0 for epochs 0-29, linear release over epochs 30-49, normal from epoch 50; blocks 0-7, embeddings, norm and head train from the start; nothing else changed" \
    ...
    --release_blocks "8,9,10,11" --release_start 30 --release_end 50 \
