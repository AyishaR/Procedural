...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --notes "as ftbanaper without the LayerNorm statistics (gains 1, biases 0: no slow input-side steps): exact per-block effective scales + rank-one coupled q/k sink at the kdyck prefix's per-block attention entropy; tests whether the sink replaces what the slow steps provide (ftbana-type profile alone: 76.6)" \
    ...
    --profile_spec "vitbase_runs/profile_ftbanaer.json" \
