...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --notes "ksd blocks 0-7 (reference ksd ftb4i 80.05), timm random + statistics of the checkpoint, no checkpoint at init; (2) = (1) + v, proj, fc2 write matched as the late lever does: one scalar per sublayer so that the attention and MLP write ratios equal the prefix's (16 targets, 256 training images, utils.calibrate_joint_statistics write_ratio), attention factor split evenly over v and proj (--scale_weights q,k,fc1 --write_ratio v,proj,fc2)" \
    ...
    --profile_spec "vitbase_runs/profile_ftbanakpeb7w.json" \
