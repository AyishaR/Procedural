#!/bin/bash
#SBATCH --job-name ftbanakperg0b7w
...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --enable_wandb true \
    --project "vit base kdyck shuffle" \
    --wandb_entity_name "procedural_pretraining" \
    --notes "statistics-only reconstruction of the ksd prefix, blocks 0-7 (reference ksd ftb4i 80.05), no checkpoint at init: exact effective scales + 64 LayerNorm statistics + attention sink (blocks 1-7, entropy) + fc1 gate (blocks 1-7, mean pre-activation) + block-0 common write (token cosine), as ftbanakperg0b7; the arms of this family differ ONLY in how the write path v / proj / fc2 is scaled: (2) v, proj, fc2 write-matched to the prefix's per-block write ratios as the late lever does, attention factor split evenly over v and proj; block 0's fc2 solved for write ratio and token cosine jointly (--scale_weights q,k,fc1 --write_ratio v,proj,fc2 --qk_entropy --fc1_gate --common_write)" \
    ...
    --profile_spec "vitbase_runs/profile_ftbanakperg0b7w.json" \
