...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --notes "ftb3b in bf16, one seed: precision-matched reference for ftb3bpl (docs/late_lever_measured_profile_brief.md). Same init as the launched ftb3b (kdyck4 ckpt, blocks 0-8 random, blocks 9-11 upscale_random_match_delta_norms at the measured targets, 5000 calibration images seed 0) on the current run template (SSD data copy, in-job retries, bf16)" \
