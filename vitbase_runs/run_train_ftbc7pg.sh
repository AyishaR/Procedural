...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --notes "mechanism study wave 1 (docs/early_lever_mechanism_plan.md): profile + rank-one mean-pre-activation gate, no sink; kdyck, blocks 0-7, committed protocol; 40-epoch screen on the unchanged 300-epoch schedule" \
    ...
    --profile_spec "vitbase_runs/profile_ftbc7pg.json" \
