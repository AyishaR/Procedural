#!/bin/bash
# ftbsv: proc's SPECTRUM, random singular DIRECTIONS in blocks 0-8. Base is ftb3i (proc 0-8, random 9-11); the spectral
#   surgery preserves ||W||_F on every matrix, so this arm is indistinguishable from ftb3i
#   by value_write and by every other norm-based quantity in docs 0. See docs 0 (2026-09-02)
#   and plots/verify_spectral_init.py.
#SBATCH --job-name ftbsv
...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --custom_init_type "spectral" \
    --custom_init_blocks "0,1,2,3,4,5,6,7,8" \
    --spectral_values "keep" \
    --spectral_basis "random" \
