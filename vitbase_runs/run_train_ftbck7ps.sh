...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --notes "mechanism study wave 1 (docs/early_lever_mechanism_plan.md): profile + rank-one sink (entropy targets), no gate; ksd, blocks 0-7, committed protocol" \
    ...
    --profile_spec "vitbase_runs/profile_ftbck7ps.json" \
