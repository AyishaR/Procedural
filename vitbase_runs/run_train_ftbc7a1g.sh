...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --notes "mechanism study wave 1 (docs/early_lever_mechanism_plan.md): as a1, and additionally lr x m on the LayerNorm gains of blocks 0-7 (m < 1: the small gains are made as slow as timm's); kdyck, blocks 0-7, committed protocol; 40-epoch screen on the unchanged 300-epoch schedule" \
    ...
    --lr_scale_json "vitbase_runs/lrscale_ftbc7a1g.json" \
