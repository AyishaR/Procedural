...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --notes "ftbanaks with a weaker sink: per-block q-bias norm chosen so the attention entropy at init is 1.7 nats in every block 1-8 (the ksd prefix's mean entropy at epoch 9, i.e. the sink after it has started to relax), instead of the prefix's init values (0.17-2.97); otherwise ftbanak" \
    ...
    --profile_spec "vitbase_runs/profile_ftbanaksw.json" \
