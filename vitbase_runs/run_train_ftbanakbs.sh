...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --notes "ftbanakb (ksd recipe + fc1 bias gate) with the gate biases at lr x0.02 (lrscale_slowgate.json): the persistent-gate control without a sink" \
