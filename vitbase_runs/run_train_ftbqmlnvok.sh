...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --notes "ftbqmlnvo recipe applied to the ksd checkpoint (pr_vitb_ksd): per-matrix empirical quantile match in blocks 0-8 (v slice against its own distribution, q/k pooled), LayerNorm 1-D tensors matched; blocks 9-11 random" \
    ...
    --pr_notes "" \
    --skip_norm true \
    --init_method "quantile_match_target_blocks" \
    --quantile_1d_mode "layernorm" \
    --quantile_qkv_mode "v_only" \
