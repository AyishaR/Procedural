...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --notes "ftbanap with LN gains ~ N(1, 0.25) and no weight compensation (anisotropy only; weights as ftbana): timm random, per-slice rms multipliers (block 0 + linear ramp 1-8) from vitbase_runs/profile_ftbanau.json, LN=1, biases 0, no checkpoint" \
    ...
    --profile_spec "vitbase_runs/profile_ftbanau.json" \
