...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --notes "mechanism study wave 2, converse of C1 by learning order (docs/early_lever_mechanism_plan.md): plain timm random init, blocks 0-7 frozen (lr factor 0) for epochs 0-29, linear release to the schedule value over epochs 30-49, normal from epoch 50; nothing else changed" \
    ...
    --release_blocks "0,1,2,3,4,5,6,7" --release_start 30 --release_end 50 \
