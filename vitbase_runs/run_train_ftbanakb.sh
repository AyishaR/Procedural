...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --notes "ftbanak (ksd 18 ramp numbers + 36 sampled LN statistics, blocks 0-8) plus one fc1 bias per block 1-8 (8 numbers) shifting the MLP pre-activations so the fraction of positive units matches the ksd prefix (0.06-0.16): the checkpoint-free stand-in for the fc1/stream alignment that silences the GELU in both procedural prefixes; blocks 9-11 random" \
    ...
    --profile_spec "vitbase_runs/profile_ftbanakb.json" \
