...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --notes "statistics-only reconstruction of the kdyck prefix (extract_profile.py --gain_fold exact --exact --qk_entropy --fc1_gate --common_write): exact per-block effective scales (54) + LayerNorm statistics (72, inline) + three joint statistics realised as rank-one components with every effective scale preserved, calibrated at init on 256 training images (utils.calibrate_joint_statistics): block 0 fc2 mean column -> token cosine of its output; blocks 1-8 coupled q/k sink -> attention entropy; blocks 1-8 fc1 mean row -> mean pre-activation (GELU gate); blocks 0-8, random 9-11, no checkpoint at init" \
    ...
    --profile_spec "vitbase_runs/profile_ftbanaperg0.json" \
