...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --notes "least-processed second-moment recipe of the ksd checkpoint, blocks 0-7 (reference ksd ftb4i): 48 exact per-block effective scales + 64 inline LayerNorm statistics (extract_profile.py --blocks 0-7 --gain_fold exact --exact on pr_vitb_ksd); blocks 0-7, random 8-11, no checkpoint at init; base of ftbanakperg0b7" \
    ...
    --profile_spec "vitbase_runs/profile_ftbanakpeb7.json" \
