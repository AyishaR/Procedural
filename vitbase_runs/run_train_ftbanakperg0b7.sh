#!/bin/bash
#SBATCH --job-name ftbanakperg0b7
...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --notes "statistics-only reconstruction of the ksd prefix ftb4i (ksd blocks 0-7), same procedure as ftbanaperg0b7 applied to pr_vitb_ksd (extract_profile.py --blocks 0-7 --gain_fold exact --exact --qk_entropy --fc1_gate --common_write): 48 exact effective scales + 64 LayerNorm statistics + 15 joint-statistic targets as rank-one components; blocks 0-7, random 8-11, no checkpoint at init" \
    ...
    --profile_spec "vitbase_runs/profile_ftbanakperg0b7.json" \
