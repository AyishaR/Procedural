...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --notes "write-profile-matched ksd recipe: ftbanak (ksd per-slice scales for q,k,v,fc1 + sampled ksd LN stats) with proj/fc2 (block 0 + ramp 1-8) refitted so the forward write-ratio profile in blocks 0-8 matches the ksd checkpoint (block 0 attn 3.2 / mlp 18, blocks 1-8 ~0.1-0.2); timm random elsewhere, no checkpoint" \
    ...
    --profile_spec "vitbase_runs/profile_ftbanakw.json" \
