...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --notes "v-inclusive ftbrhosl: timm random init with per-tensor lr / multiplier (wd x multiplier) on proj/fc2 AND on the v rows of the fused qkv (row lr mask) of blocks 9-11, multipliers = ftbrho init factors v=proj 3.01/4.60/7.61, fc2 9.42/27.47/81.71: random-scale writes incl. v, ftbrho's slow Adam relative steps; fp16 as ftbrhosl" \
    ...
    --skip_norm true \
    --init_method "default" \
    --lr_scale_json "vitbase_runs/lrscale_ftbrhoslv.json" \
