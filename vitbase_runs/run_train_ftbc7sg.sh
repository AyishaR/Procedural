...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --notes "mechanism study wave 1 (docs/early_lever_mechanism_plan.md): structure only: sink + mean gate on a plain timm init (no scales, no LayerNorm statistics); components add to the matrix (renormalize false); kdyck, blocks 0-7, committed protocol" \
