...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --notes "statistics-only reconstruction of the ksd prefix, same procedure as ftbanaperg0 applied to pr_vitb_ksd (extract_profile.py --gain_fold exact --exact --qk_entropy --fc1_gate --common_write): 54 exact effective scales + 72 LayerNorm statistics + 17 joint-statistic targets as rank-one components; blocks 0-8, random 9-11, no checkpoint at init" \
    ...
    --profile_spec "vitbase_runs/profile_ftbanakperg0.json" \
