...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --notes "mechanism study wave 2, C1 (docs/early_lever_mechanism_plan.md): committed kdyck init (bit-identical) + auxiliary cross-entropy on the block-7 lens, weight 0.3, epochs 0-49, then off: forces the class to be readable at block 7 early" \
