...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --notes "v-inclusive ftbrhopl: ftbrho init (v, proj x3.01/4.60/7.61, fc2 x9.42/27.47/81.71 in blocks 9-11, read off ftbrho_s0.pth) with per-tensor lr x multiplier (wd / multiplier) on proj/fc2 AND on the v rows of the fused qkv via the row lr mask: loud writes incl. v, random-init Adam relative steps; bf16 as ftbrhopl" \
    ...
    --profile_spec "vitbase_runs/profile_ftbrhoplv.json" \
    --lr_scale_json "vitbase_runs/lrscale_ftbrhoplv.json" \
