...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --notes "ftbrhop with per-tensor lr x multiplier (wd / multiplier) on the scaled proj/fc2 of blocks 9-11: loud writes, random-init Adam relative steps (loud, not slow); bf16 autocast because the compensated steps push the block 9-11 activations past fp16 (first fp16 attempt 29626864 NaN at epoch 44)" \
    ...
    --profile_spec "vitbase_runs/profile_ftbrhop.json" \
    --lr_scale_json "vitbase_runs/lrscale_ftbrhopl.json" \
