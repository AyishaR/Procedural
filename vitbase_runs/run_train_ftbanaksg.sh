...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --notes "ftbanaks (ksd recipe + per-block attention sink) plus the fc1 bias gate of blocks 1-8 matched to the ksd prefix's fraction of positive pre-activations, with the gate biases at lr x0.02 so the gate persists (lrscale_slowgate.json); the full structural state of the prefix in 16 numbers" \
    ...
    --profile_spec "vitbase_runs/profile_ftbanaksg.json" \
    --lr_scale_json "vitbase_runs/lrscale_slowgate.json" \
