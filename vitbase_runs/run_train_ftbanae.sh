...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --notes "control for ftbanaer: exact per-block effective scales of the kdyck checkpoint with gains 1 and biases 0 (extract_profile.py --gain_fold exact --exact --no_layernorm), no sink; the exact-form analogue of ftbana" \
    ...
    --profile_spec "vitbase_runs/profile_ftbanae.json" \
