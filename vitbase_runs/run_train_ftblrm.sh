...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --notes "random init, lr steps matched to proc in blocks 0-8" \
    ...
    --pr_notes "" \
    --skip_norm true \
    --lr_match_ckpt "results/pr_vitb_n/pr_6066174_final.pth" \
    --lr_match_blocks "0,1,2,3,4,5,6,7,8" \
