...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --notes "ksd recipe with the q,k scales of blocks 1-8 reduced so the attention logit std at init is the kdyck recipe's 0.55 (diffuse attention) plus the persistent fc1 bias gate (lr x0.02): the kdyck route on ksd's other numbers" \
    ...
    --profile_spec "vitbase_runs/profile_ftbanakd.json" \
