...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --notes "control for the recipe conventions: ftbanap with the checkpoint's own effective scales -- exact gain folding rms(W diag gamma) instead of rms(W)*rms(gamma) (4-11% larger on the input side) and q/k measured separately (1.46->1.51, 1.82->1.67) instead of the twin's pooled 1.32; fc2 end 0.95 kept; LN statistics inline (extract_profile.py --gain_fold exact --query_key separate --fc2_end 0.95); blocks 0-8, random 9-11" \
    ...
    --profile_spec "vitbase_runs/profile_ftbanapx.json" \
