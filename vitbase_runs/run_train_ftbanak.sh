...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --notes "ftbanap recipe fitted to the ksd checkpoint: block 0 + linear ramp 1-8 per slice (18 numbers) and Gaussian-sampled LN gains/biases (per-block mean/std of pr_vitb_ksd); timm random, per-slice rms multipliers (block 0 + linear ramp 1-8) from vitbase_runs/profile_ftbanak.json, LN=1, biases 0, no checkpoint" \
    ...
    --profile_spec "vitbase_runs/profile_ftbanak.json" \
