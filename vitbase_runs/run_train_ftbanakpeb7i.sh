...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --notes "ksd blocks 0-7 (reference ksd ftb4i 80.05), timm random + statistics of the checkpoint, no checkpoint at init; (1) input side only: exact effective scales of q, k, fc1 (24) + 64 LayerNorm statistics; v, proj, fc2 are timm's (extract_profile.py --blocks 0-7 --gain_fold exact --exact --scale_weights q,k,fc1)" \
    ...
    --profile_spec "vitbase_runs/profile_ftbanakpeb7i.json" \
