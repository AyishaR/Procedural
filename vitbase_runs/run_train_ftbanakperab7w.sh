#!/bin/bash
#SBATCH --job-name ftbanakperab7w
...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --notes "statistics-only recipe for ksd blocks 0-7 (reference ksd ftb4i 80.05), timm random + numbers read from the checkpoint, no checkpoint at init: exact effective scales + 64 LayerNorm statistics; q/k: rank-one sink calibrated to the prefix's attention entropy (blocks 1-7); fc1: rank-one gate calibrated to the prefix's FRACTION OF ACTIVE fc1 UNITS (blocks 1-7); no block-0 common write; calibration order per block: entropy, attention write, active fraction, MLP write; (2) v, proj, fc2 write-matched to the prefix's write ratios, attention factor split evenly over v and proj (--scale_weights q,k,fc1 --qk_entropy --fc1_gate --write_ratio v,proj,fc2)" \
    ...
    --profile_spec "vitbase_runs/profile_ftbanakperab7w.json" \
