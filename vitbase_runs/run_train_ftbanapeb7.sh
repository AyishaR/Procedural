...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --notes "least-processed second-moment recipe of the kdyck checkpoint, blocks 0-7 (reference ftb4i): exact per-block effective scales (48 numbers, rms(W diag gamma) folding, q/k separate, no ramp, no fc2 correction) + 64 inline LayerNorm statistics (extract_profile.py --blocks 0-7 --gain_fold exact --exact); blocks 0-7, random 8-11, no checkpoint at init; base of ftbanaperg0b7" \
    ...
    --profile_spec "vitbase_runs/profile_ftbanapeb7.json" \
