...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:$MASTER_PORT --nproc_per_node=$SLURM_GPUS_ON_NODE main.py \
    ...
    --notes "mechanism study wave 2, converse of C1 (docs/early_lever_mechanism_plan.md): plain timm random init + suppress penalty on the block-7 lens (lens pushed to the uniform distribution, final norm and head detached), weight 0.3, epochs 0-49, then off" \
    ...
    --aux_lens_block 7 --aux_lens_mode suppress --aux_lens_weight 0.3 --aux_lens_until 50 \
