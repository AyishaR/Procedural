...
from layer_wise_stats import (
    StreamingAllProbeStats,
    save_streaming_stats_checkpoint,
    load_streaming_stats_checkpoint,
)
...
def get_args_parser():
    ...
    parser.add_argument('--probe_blocks', default='11', type=str)
...
def main(args):
    ...
    vit_small_probe_dims = {
        ...
        "sample_wise_cls": len(dataset_val),
        "token_wise_cls": 384,
    }
    blocks_list = {
        "default": list(range(args.num_blocks)),
        "sample_wise_cls": [int(ii) for ii in args.probe_blocks.split(",")] if args.probe_blocks != "" else list(range(args.num_blocks))
    }
    ...
    if "pr" in args.initialize:
        master_epoch= 0 # for procedural pretraining, we only calculate the probe stats at the end of training, so we can set master_epoch to 0 since we will only run for 1 epoch in the loop below
    else:
    ...
    if args.probe_mode=="sample_wise_cls":
        data_loader_used = data_loader_val
        print(f"Number of samples in validation set to collect probe stats on: {len(data_loader_val.dataset)}")
    else:
        data_loader_used = data_loader_train
        print(f"Number of samples in training set to collect probe stats on: {len(dataset_train)}")

    print(f"Collecting probe stats on {'validation' if args.probe_mode=='sample_wise_cls' else 'training'} data with {len(data_loader_used)} batches ...")
    probe_names = [
            label
            for ii in blocks_list.get(args.probe_mode, blocks_list["default"])
            for label in (get_probe_label(2*ii), get_probe_label(2*ii+1))
        ]
    stats_device = "cpu"
    stream_stats = StreamingAllProbeStats(
        feature_dims=[vit_small_probe_dims[args.probe_mode]] * (len(probe_names)), # for each block, we collect stats on both the attention output and the block output, so multiply num_blocks by 2
    ...
    if args.probe_mode!="sample_wise_cls":
        start_epoch, start_batch_idx = load_streaming_stats_checkpoint(stream_stats, args.output_dir, master_epoch=master_epoch)
    else:
    ...
    for epoch in range(1):
        ...
        if args.distributed:
            data_loader_used.sampler.set_epoch(epoch)
        ...
        for batch_idx, (images, targets) in enumerate(data_loader_used):
            print(f"Processing batch {batch_idx}/{len(data_loader_used)} ...", flush=True)
            ...
            probe_features = []
            for bi in blocks_list.get(args.probe_mode, blocks_list["default"]):
            ...
            # print(f"Selected probe features for batch {batch_idx}: {acts[11].keys()}")
            # probe_features = [select_probe_feature(acts[11]["blk"], mode=args.probe_mode)]
            # print(f"Probe features shape for batch {batch_idx}: {probe_features[0].shape}")

            if args.probe_mode!="sample_wise_cls":
                stream_stats.update(probe_features, targets)
            else:
                probe_features_all.extend(probe_features)
                targets_all.extend(targets)
                print(targets_all)

            if args.output_dir and ((batch_idx + 1) % 20 == 0) and args.probe_mode!="sample_wise_cls":
        ...
        if args.probe_mode!="sample_wise_cls":
            save_streaming_stats_checkpoint(
                ...
                master_epoch=master_epoch,
                epoch=epoch,
                batch_idx=len(data_loader_used),
                rank=rank,
            )

        if args.probe_mode=="sample_wise_cls":
            probe_features_all = torch.cat(probe_features_all, dim=0).unsqueeze(0)
            targets_all = torch.stack(targets_all, dim=0)
            print(f"Final probe features shape after concatenating all batches: {probe_features_all.shape}")
            print(f"Final targets shape after concatenating all batches: {targets_all.shape}")
            # for sample-wise cls probe, we need to gather the features and targets from all batches and then calculate the metrics at the end of the epoch
            stream_stats.to_device(device)
            stream_stats.update(probe_features_all, targets_all)


    if args.distributed:
        print("Waiting for all processes to finish collecting stats before calculating final metrics...")
    ...
    if args.probe_mode!="sample_wise_cls":
        stream_stats.sync_ddp()

    metrics = stream_stats.finalize(eps=1e-3)
    print(f"Final probe metrics at epoch {master_epoch}: {metrics}")

    if (not args.distributed) or dist.get_rank() == 0:
        metrics_path = os.path.join(args.output_dir, f"probe_E{master_epoch:03d}_representation_metrics_1e3.json")
        ...
        with open(metrics_path, "w", encoding="utf-8") as f:
            print(f"Saving probe metrics to {metrics_path} ...")
