"""Verify the v-inclusive late-lever step-size arms (blocks 9-11; docs/proc_init_recipe.md section 13):
  ftbrhoplv  ftbrho's init (v, proj x sqrt(a_b); fc2 x f_b) via profile_ftbrhoplv.json "extra" + lr x the same multipliers
             (proj, fc2 as scalar groups; the v rows of the fused qkv via the row lr mask)   -- loud writes, normal relative steps
  ftbrhoslv  timm init + lr x the reciprocals on the same tensors                             -- random writes, slow relative steps
  ftbrho     (exists, n = 3) is the loud + slow cell; random (n = 3) the random + normal cell.
Checks
  ...
  B  the reconstruction equals ftbrho's own init dump (results/init_dumps/ftbrho_s0.pth) to the rounding of the multipliers.
  C  lrscale_ftbrhoplv.json: scalar entries == the profile multipliers, row entries == [[1536, 2304, v_b]] exactly;
     lrscale_ftbrhoslv.json: the reciprocals (relative deviation < 1e-5); no other tensor named.
  D  --dump_pl / --dump_sl (main.py's own init, plots/dump_init.py): ftbrhoplv dump == reconstruction bit for bit; ftbrhoslv dump ==
     timm (seed 0) bit for bit.
  E  write ratios at init on 64 training images: the reconstruction and ftbrho's dump agree per block (attention, MLP).
...
ap.add_argument("--profile", default="vitbase_runs/profile_ftbrhoplv.json"); ap.add_argument("--lr_pl", default="vitbase_runs/lrscale_ftbrhoplv.json")
ap.add_argument("--lr_sl", default="vitbase_runs/lrscale_ftbrhoslv.json"); ap.add_argument("--ftbrho", default="results/init_dumps/ftbrho_s0.pth")
a = ap.parse_args()
ok = True
...
print("B. reconstruction vs ftbrho's own init dump")
if os.path.exists(a.ftbrho):
    rho = torch.load(a.ftbrho, map_location="cpu", weights_only=False); rho = rho.get("model", rho)
    ...
    check("ftbrho's dump is timm outside blocks 9-11 (same seed 0 random init)", same_out)
else:
    check("ftbrho dump present", False, a.ftbrho)
...
check("ftbrhoplv: lambda = the init multipliers (proj, fc2 scalars; v rows [1536, 2304) exactly)", dev_pl == 0 and rows_pl, f"scalar deviation {dev_pl:.1e}")
...
check("ftbrhoslv: lambda = the reciprocals (rows [1536, 2304))", dev_sl < 1e-5 and rows_sl, f"max relative deviation {dev_sl:.1e}")
...
if a.dump_pl and os.path.exists(a.dump_pl):
    ...
    check("ftbrhoplv dump == reconstruction, bit for bit, every tensor", all(torch.equal(d[k], rec[k]) for k in rec), f"{len(rec)} tensors")
else:
...
if a.dump_sl and os.path.exists(a.dump_sl):
    ...
    check("ftbrhoslv dump == timm seed 0, bit for bit, every tensor", all(torch.equal(d[k], timm[k]) for k in timm), f"{len(timm)} tensors")
else:
    print("  (no --dump_sl: skipped)")

print("E. write ratios at init on 64 training images (reconstruction vs ftbrho's dump)")
train_dir = os.path.join(a.data_path, "train")
if os.path.isdir(train_dir) and os.path.exists(a.ftbrho):
    ...
    def ratios(sd):
        ...
        return out
    r_rec, r_rho = ratios(rec), ratios(rho)
    worst = max(abs(r_rec[b][i] / r_rho[b][i] - 1) for b in BLOCKS for i in (0, 1))
    for b in BLOCKS:
        print(f"    block {b}: attention {r_rec[b][0]:.4f} (ftbrho {r_rho[b][0]:.4f})   MLP {r_rec[b][1]:.4f} (ftbrho {r_rho[b][1]:.4f})")
    check("reconstruction and ftbrho agree per block", worst < 1e-3, f"max relative deviation {worst:.1e}")
else:
    print("  (no training images or no ftbrho dump: skipped)")
